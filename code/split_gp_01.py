...
def parse_args():
    ...
    parser.add_argument("--rounds", type=int, default=120,
                        help="Number of global communication rounds (Paper: 120 for MNIST/FMNIST, 800 for CIFAR10)")
    parser.add_argument("--local-epochs", type=int, default=1,
                        help="Epochs per client per round (paper uses 1, but may need 0.5 for MNIST to match 12 updates)")
    parser.add_argument("--max-local-updates", type=int, default=None,
                        help="Max local updates per round (paper: 12 for MNIST, 10 for CIFAR10). If set, overrides local-epochs.")
    ...
    parser.add_argument("--eth", type=float, default=0.8,
                        help="Entropy threshold (Eth) for selective offloading (Paper uses: 0.05-2.3 range)")
    ...
    parser.add_argument("--model", type=str, default="SimpleCNN",
                        help="Model type for results folder (e.g., SimpleCNN, VGG11)")
...
def evaluate_method_split(clients_phi_states,
    ...
    for k in range(K):
        base_local = create_model(model_type, in_channels=in_channels, num_classes=10, split_index=split_index).to(
            DEVICE)
...
class VGG11(nn.Module):
    """
    VGG-11 model for CIFAR10 WITH BatchNorm (needed for stable training).
    Paper split: |φ| = 972,554, |θ| = 8,258,560, |κ| = 10,250

    Note: Paper likely used BatchNorm but didn't count it in parameter reports.
    Without BatchNorm, VGG-11 is extremely unstable and won't converge properly.

    Architecture: 8 conv layers [64, 128, 256, 256, 512, 512, 512, 512] + BN + classifier
    ...
    def __init__(self, num_classes=10, split_index=4, in_channels=3, small_classifier=True):
        ...
        for v in cfg:
            if v == 'M':
                ...
                # WITH BatchNorm for stable training (critical for VGG on CIFAR-10)
                conv2d = nn.Conv2d(in_ch, v, kernel_size=3, padding=1, bias=False)  # No bias with BN
                layers.append(conv2d)
                layers.append(nn.BatchNorm2d(v))  # BatchNorm for stability
...
def create_model(model_type, in_channels, num_classes=10, split_index=None):
    ...
    if model_type.lower() == 'simplecnn':
        ...
        if split_index is None:
            split_index = 4
        model = VGG11(num_classes=num_classes, split_index=split_index, in_channels=in_channels,
...
if __name__ == "__main__":
    ...
    if MAX_LOCAL_UPDATES is None and DATASET in ['MNIST', 'FMNIST']:
        ...
        print(
            f"⚙️  Auto-setting max_local_updates={MAX_LOCAL_UPDATES} for {DATASET} (paper configuration)")  # CRITICAL: Missing assignment
    ROUNDS = args.rounds
    ...
    if "splitgp" in methods_to_run:
        ...
        if all_sweep_results:
            ...
            if not eth_data.empty:
                ...
                plt.title(f"SplitGP on {DATASET}: Comparison (γ={GAMMA}, λ={LAMBDA_SPLITGP}, Eth={best_eth:.2f})",
                          fontsize=13)
                plt.xticks(p_values)
                plt.ylim(0, 105)
                plt.grid(True, alpha=0.3)
